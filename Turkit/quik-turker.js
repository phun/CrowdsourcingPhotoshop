...
var url = "http://people.csail.mit.edu/juhokim/sinch2/";

var gettask_url = "http://people.csail.mit.edu/juhokim/sinch2/php/gettask.php";

var title = "Answer %%n%% quick search queries!";
var description = "Provide answers for these %%n%% search queries. The answers you provide will be read by someone on a mobile phone, so please limit each one to 200 words at most. You must use the browser within the page to find the answer, or else you won't be paid.";
...
// amount of time after starting to post HITs even
// if no answers are needed
var aggressiveTime = 60*3;
...
for(var i=0; i<2; i++) {
  ...
  var runningTime = (time() - startTime) / 1000;
  print("running time so far: " + runningTime);

  // If someone's interacting with the application and we don't
  // already have some active HITs, seek 1 answer optimistically.
  if(diff < 60*5 && lowAnswer > (numAnswersDesired -1) && runningTime < aggressiveTime) {
