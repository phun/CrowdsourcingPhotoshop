...
if ($video_id[0] == "c")
	...
	$list = array_merge($cid_list, $mid_list, $pid_list);
...
foreach ($list as $index=>$vid){
	$labels_data = array();
	$result = $mysqli->query("SELECT * FROM labels WHERE id = '$vid'");
	// if ($result->num_rows != 1)
	// 	echo "query error";
	// $video = $result->fetch_assoc();
	while($responses = $result->fetch_assoc()){
		$video_data[$responses["video_id"] = array(
	...
	$video_data = array();
	$result = $mysqli->query("SELECT * FROM videos WHERE id = '$vid'");
	// if ($result->num_rows != 1)
	// 	echo "query error";
	// $video = $result->fetch_assoc();
	while($responses = $result->fetch_assoc()){
		$video_data[$vid] = array(
			"video_id" => $responses["in_chi2014_set"],
			"slug" => $responses["slug"],
			"duration" => $responses["duration"],
			"title" => $responses["title"],
