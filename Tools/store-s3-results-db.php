...
function storeInDB($mysqli, $video_id, $cluster_id, $det_label_index, $det_before_index, $det_after_index){
	global $id_list;
    ...
    $det_before_index = $mysqli->escape_string($det_before_index);
    $det_after_index = $mysqli->escape_string($det_after_index);
	$success = true;
    
    echo $cluster_id . " " . $video_id . " " . $det_label_index . " " . $det_before_index . " " . $det_after_index . "<br>";
	if (!$mysqli->query("UPDATE stage2_3 SET det_before_index='$det_before_index', det_after_index='$det_after_index' WHERE id='$cluster_id' AND det_label_index='$det_label_index'"));
	  	$success = false;
	// if ($mysqli->affected_rows != 1)
	//   	$success = false;
    if ($success == false)
        echo "error" . " " . $cluster_id . " " . $video_id . " ". $det_before_index . " " . $det_after_index . "<br>";
    if ($success == true)
        $success_count += 1;
}
    	...
    	foreach($video_val as $cluster_id => $cluster_val){
    		...
    		if (is_array($cluster_val)){
    			foreach($cluster_val as $field => $field_val){
                    ...
                    else if ($field == "before_index")
                        $det_before_index = $field_val;
                    else if ($field == "after_index")
                        $det_after_index = $field_val;                    
    			}
    		}
    		...
    		// echo $new_video_id . "\n\n";
    		// echo $cluster_id . "\n\n";
    		// echo $det_before_index . "\n\n";
      //       echo $det_after_index . "\n\n";
    		// echo print_r($time_array) . "\n\n";
    		// echo print_r($desc_array) . "\n\n<br>";
    		storeInDB($mysqli, $new_video_id, $cluster_id, $det_label_index, $det_before_index, $det_after_index);
