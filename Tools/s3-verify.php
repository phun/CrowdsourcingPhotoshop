...
$s23_data = array();
$result = $mysqli->query("SELECT * FROM stage2_3_v1");
while($responses = $result->fetch_assoc()){
...
<head>
	...
	.turker-input .before-image, .turker-input .after-image{
		float: left;
		width: 250px;
		height: 180px;
	}
	...
	function displaySingleChoice(slug, second, $el){
		...
		var start = <?php echo json_encode($s23_data[$id]["time"]); ?> - 10;
		console.log(start);		
		...
		var vid = <?php echo json_encode($s23_data[$id]["video_id"]); ?>,
			s23_data = <?php echo json_encode($s23_data); ?>,
			video_data = <?php echo json_encode($video_data); ?>,
			tname = <?php echo json_encode($s23_data[$id]["label"]); ?>,
		...
		$(document).ready(function() {
			...
			console.log(genre, vid, tname, turk_data, cluster_id, video);		
      		...
      		for (var cid in turk_data){
      			...
      			var slug = video_data[short_vid]["slug"];
      			console.log(short_vid, slug, s23_data[cid]["label"]);
      			for (var i in turk_data[cid]){
      				var label = turk_data[cid][i];
      				console.log(label["before_index"], label["after_index"]);
      				var dom_id = cid + "-" + i;
      				var $el = $("<div/>").attr("id", dom_id).addClass("turker-input")
      							.append("<div>" + s23_data[cid]["label"] + "</div>")
